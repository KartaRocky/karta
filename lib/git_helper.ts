...
export const findFileGitLab = async (
...
export const findBlobSchemasFileGitLab = async (
  fileFilter: string
): Promise<BlobSchema[]> => {
  const api = new Gitlab({
    host: process.env.BASE_GIT_URL,
...
export const findProjectByIdGitLab = async (
  projectId: string | number
...
export const findBlobSchemasFileGithub = async (
  fileFilter: string
): Promise<RepoFileSchema[]> => {
...
export const findFileContentInGitHub = async (
  owner: string,
  repo: string,
  path: string
): Promise<KartaDependency[]> => {
  const api = new Octokit({
    baseUrl: process.env.BASE_GIT_URL,
    auth: process.env.PRIVATE_TOKEN,
  });
