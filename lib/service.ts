...
import { findBlobSchemasFileGithub, findBlobSchemasFileGitLab, findFileContentInGitHub, findFileGitLab, findProjectByIdGitLab } from './git_helper';
...
export async function findKartaFileAndUpdateDatabase() {
  ...
  if (process.env.GIT_TYPE === 'github') {
    ...
    blobSchemas.forEach(async (blobSchema) => {
      await saveSource({url: blobSchema.repoUrl, name: blobSchema.repoName, path_with_namespace: blobSchema.repoFullName})

      const dependencies = await findFileContentInGitHub(blobSchema.repoOwner, blobSchema.repoName, blobSchema.filePath)
